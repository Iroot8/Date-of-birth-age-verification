...
def save_to_log(birth_date, age):
    """Сохранение результата проверки в лог-файл"""
    now = datetime.now().strftime("%d.%m.%Y %H:%M:%S")
    with open(LOG_FILE, "a", encoding="utf-8") as file:
        file.write(f"{now} | Дата рождения: {birth_date} | Возраст: {age} полных лет\n")
...
def check_age():
    ...
    try:
        ...
        if (today.month, today.day) < (birth.month, birth.day):
            age -= 1
        result_text = f"Ваш возраст: {age} полных лет."
        text_result.config(text=result_text)
        
        # Сохраняем результат в лог-файл
        save_to_log(date_str, age)
